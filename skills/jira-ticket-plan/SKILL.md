...
- 狀態轉移交給 commit hook 處理，**不要**在此手動轉票。提交後會有 hook 提示把「待辦事項／計畫階段」轉「進行中」（Claude Code 走 `.claude/hooks/post-commit-jira.py`；Codex 與純 git 走 `.git/hooks/post-commit`，由 `scripts/install-git-hooks.sh` 安裝）
